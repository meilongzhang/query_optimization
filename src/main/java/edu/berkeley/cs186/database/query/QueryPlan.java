...
public class QueryPlan {
    ...
    public QueryOperator minCostSingleAccess(String table) {
        ...
        int sscost = minOp.estimateIOCost();
        int exclude = -1;
        List<Integer> preds = getEligibleIndexColumns(table);
        if (preds.size() != 0) {
            for (Integer i : preds) {
                SelectPredicate p = this.selectPredicates.get(i);
                IndexScanOperator tOp = new IndexScanOperator(this.transaction, table, p.column, p.operator, p.value);
                int tOpcost = tOp.estimateIOCost();
                if (tOpcost < sscost) {
                    sscost = tOpcost;
                    minOp = tOp;
                    exclude = i.intValue();
    ...
    public Map<Set<String>, QueryOperator> minCostJoins(
        ...
        for (Set<String> s : prevMap.keySet()) {
            ...
            for (JoinPredicate j : this.joinPredicates) {
                ...
                //
                String lt = j.leftTable;
                String lc = j.leftColumn;
                String rt = j.rightTable;
                String rc = j.rightColumn;
                QueryOperator lq = null;
                QueryOperator rq = null;
                ...
                if (s.contains(lt) && !s.contains(rt)) {
                    for (Set<String> rSet : pass1Map.keySet()) {
                        if (rSet.contains(rt)) {
                            rq = pass1Map.get(rSet);
                        }
                    }
                }
                // Case 2: The set contains right table but not left, use pass1Map
                // to fetch an operator to access the leftTable.
                else if (s.contains(rt) && !s.contains(lt)) {
                    for (Set<String> lSet : pass1Map.keySet()) {
                        if (lSet.contains(lt)) {
                            lq = pass1Map.get(lSet);
                        }
                    }
                }
                ...
                if (lq != null) {
                    QueryOperator minJoin = minCostJoinType(lq, prevMap.get(s), lc, rc);
                    Set<String> newSet = new HashSet<String>();
                    newSet.addAll(s);
                    newSet.add(lt);

                    result.put(newSet, minJoin);
                } else if (rq != null) {
                    QueryOperator minJoin = minCostJoinType(prevMap.get(s), rq, lc, rc);
                    Set<String> newSet = new HashSet<String>();
                    newSet.addAll(s);
                    newSet.add(rt);
    ...
    public Iterator<Record> execute() {
        ...
        // operator.
        Map<Set<String>, QueryOperator> mapping = new HashMap<>();
        Map<Set<String>, QueryOperator> map1 = new HashMap<>();
        QueryOperator q = null;
        for (String table : this.tableNames) {
            q = minCostSingleAccess(table);
        ...
        if (this.tableNames.size() > 1) {
            for (int i = 0; i < this.tableNames.size() - 1; i++) {
                mapping = minCostJoins(mapping, map1);
            }
            this.finalOperator = minCostOperator(mapping);
        } else {
